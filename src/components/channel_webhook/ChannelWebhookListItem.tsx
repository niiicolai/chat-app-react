...
const ChannelWebhookListItem = (props: ChannelWebhookListItemProps): JSX.Element => {
    const { webhook, channel, setWebhookEdit, setWebhookDelete, setWebhookTest } = props;
    ...
    return (
        <li className="flex flex-col gap-1 border border-gray-800 p-3 rounded-md break-all" data-testid="channel-webhook-list-item">
            ...
            <div className="bg-gray-700 p-2 rounded-md flex gap-1">
                <Button type="primary" display="h-5 w-5 flex items-center justify-center" title="Test webhook" testId="channel-webhook-test-button" onClick={() => setWebhookTest(webhook)} button="button" slot={
                    <PaperPlaneIcon fill="white" width=".8em" />
                } />
                <Button type="primary" display="h-5 w-5 flex items-center justify-center" title="Edit webhook" testId="channel-webhook-edit-button" onClick={() => setWebhookEdit(webhook)} button="button" slot={
                    <PenIcon fill="white" width=".8em" />
                } />
                <Button type="error" display="h-5 w-5 flex items-center justify-center" button="button" title="Delete webhook" testId="channel-webhook-delete-button" onClick={() => setWebhookDelete(webhook)} slot={
